...
__TODO__: [Andrea] Since the MR8 departure, the whole fMRI procedure (and possibly other parts of the Wiki, e.g., button boxes, screen size, etc.) are now obsolete. We need to update these pages with the new info from the MRI facility. Ron has a booklet with all the information on the new system which would be nice to get/copy. Some extra info: monitor physical size: 700 * 395 mm; resolution: 1920 * 1080; distance (eye-to-screen): 185 mm; new trigger code: t; new button codes: left hand (top to bottom): blue = e, yellow = w, green = n, red = d; right hand (top to bottom): blue = b, yellow = y, green = g, red = r. The check-in for participants is also slightly different, and the location is different. It may be worth having a chat with Floor, since she just started testing.
-->
