...
!!! Tip
    Encryption keys can be stored in [Passwordstate](https://admin.kuleuven.be/icts/services/passwordstate), with the keys shared with the appropriate persons within the group.
...
If applicable, **get acquainted with specific testing procedures** ([EEG](../eeg/eeg-acquisition.md)/[fMRI](../fmri/fmri-procedure.md)) and start collecting your data. 

For behavioural experiments, this can either be using one of the specified online platforms (Pavlovia, Meadows, Prolific, Microsoft Forms), directly from your PC or using paper questionnaires. 
...
Store **physically signed ICFs** properly in the designated secure location (i.e., a locked cupboard in your office). They must be stored for a retention period of at least 10 years (SMEC) or 25 years (EC). 
...
!!! warning "Important"
    These are not part of the research data and need to be stored separately and encrypted to avoid reidentification of the participants ([how to encrypt](https://admin.kuleuven.be/icts/english/research/datamgmtpract/ape/encryption)).

    These data should be deleted once they are no longer relevant (e.g., when the study is closed), unless participants explicitly agreed to store them longer for a specific purpose (discuss this with your PI before deleting them).
...
If the different steps below are followed in quick succession (or certain steps are done several times), repeated downloads/uploads make less sense and might be cumbersome. Therefore, they may be omitted, but researchers should keep in mind that ManGO is the main storage location for the research data and should thus be kept up to date.​
