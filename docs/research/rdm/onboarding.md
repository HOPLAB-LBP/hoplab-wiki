# Researcher onboarding checklist

Hi and welcome to Hoplab! Here’s a list of things to think about if you are just starting out. It links to different Wiki pages so every step should speak for itself. If it doesn’t, do not hesitate to contact [Klara](https://www.kuleuven.be/wieiswie/nl/person/00116743) to help you set up. 

You can follow the checklist step by step below or [download a personal copy](https://kuleuven.sharepoint.com/:w:/r/sites/T0005824-Hoplab/Shared%20Documents/Hoplab/Research/RDM/Researcher-onboarding-checklist_WIP.docx?d=w671ad86c962d4a2092ed494e51e0cd04&csf=1&web=1&e=FXG5oL).

??? note "🗂️ Administrative and access setup"
    | Task | Information | Done |
    |------|-------------|------|
    | Activate your KU Leuven account | Find more information [here](../../get-started/admin-procedures.md#activate-your-ku-leuven-account) | ☐ |
    | Get a key to your new office | Find more information [here](../../get-started/practical-setup.md#keys) | ☐ |
    | Order laptop and required peripherals | Find more information [here](../../get-started/practical-setup.md#office-set-up) | ☐ |
    | Get access to all communication channels | See the overview [here](../../get-started/computer-setup.md#communication-channels) | ☐ |
    | Get access to relevant physical spaces | e.g., the [MRI scanner room](../fmri/fmri-get-started.md#gain-access-to-mr-facilities) | ☐ |
    | Get added to the lab’s [meeting schedule](../../outreach.md#schedule) | Ask [Klara](https://www.kuleuven.be/wieiswie/nl/person/00116743) to add you to the planning | ☐ |
    | Appear as a member on the [Hoplab website](https://www.hoplab.be) | Find inspiration from your colleagues [here](https://www.hoplab.be/people/) | ☐ |
    | Read getting started section on Wiki | Here’s a [quick link](../../get-started/index.md) | ☐ |
    | Receive and register [HSE orientation](../../get-started/index.md/#health-safety-and-environment) | Inform [Zsuzsa](https://www.kuleuven.be/wieiswie/nl/person/00048902) and [Ying](https://www.kuleuven.be/wieiswie/en/person/00098752) about your start-up | ☐ |

??? note "📊 Research data management"
    | Task | Information | Done |
    |------|-------------|------|
    | Get acquainted with the [current RDM guidelines](current.md) | Please check these regularly for updates | ☐ |
    | Get acquainted with our [offboarding expectations](offboarding.md) | To do it right from the start :) | ☐ |
    | Read [guidelines regarding documentation](offboarding.md#documentation-expectations) | To make your data FAIR | ☐ |
    | Get acquainted with the [BIDS standard](https://bids-specification.readthedocs.io/en/stable/) | [Hoplab example for fMRI datasets](../fmri/analysis/fmri-bids-conversion.md) | ☐ |
    | Get access to your [Sharepoint](https://icts.kuleuven.be/sc/english/collaboration/sharepointonline) site (day-to-day work) | Ask [Klara](https://www.kuleuven.be/wieiswie/nl/person/00116743) to request one with dICT | ☐ |
    | Get access to [ManGO](https://rdm-docs.icts.kuleuven.be/mango/index) (active research data) | Ask [Klara](https://www.kuleuven.be/wieiswie/nl/person/00116743) to create a new folder for you | ☐ |
    | Get access to [FriGO](https://rdm-docs.icts.kuleuven.be/frigo/index) (data archive) | (This is currently still in pilot phase, tbc) | ☐ |
    | Read our [guidelines regarding data sharing](./RDR_sharing.md) | Useful links: [Hoplab OSF](https://osf.io/g23pm/), [RDR manual](https://www.kuleuven.be/rdm/en/rdr/manual) | ☐ |

??? note "🛡️ Ethics and data protection"
    | Task | Information | Done |
    |------|-------------|------|
    | Get [ethical approval](../ethics/index.md#general-information) (if needed) | Don't forget to update our [ethics tracker](../ethics/index.md#ethics-tracker) | ☐ |
    | Fulfill e-learning *GDPR and privacy in research* | [Link](https://admin.kuleuven.be/privacy/en/studpers/index) to e-learning | ☐ |
    | Obtain your GCP certificate (if applicable) | [Link](https://gbiomed.kuleuven.be/english/ctc/intern/ctc-academy/ich-gcp-e6-r2-training-certification) to e-learning | ☐ |
    | Make sure you know [how to handle confidential info](./SOPs.md) | ICFs, contact info, pseudonymization keys, etc. | ☐ |
    | Get to know our [participant database](https://forms.office.com/Pages/ResponsePage.aspx?id=m1hzOUCetU6ADrC2OD0WIZquopqLrtBFjNZoWiO2ApRUNFdCTzlBM1AxTTBMRENONjNHNzFYTjEwOS4u) (healthy adults) | Contact [Silke](https://www.kuleuven.be/wieiswie/nl/person/00170487) or [Klara](https://www.kuleuven.be/wieiswie/en/person/00116743) if you need participants| ☐ |

??? note "🛠️ Tools and research workflows"
    | Task | Information | Done |
    |------|-------------|------|
    | Activate your Matlab license (if applicable) | Find more information [here](../../get-started/computer-setup.md#installing-matlab) | ☐ |
    | Read about [good coding practices](../coding/index.md) | See also the [Hoplab Github](https://github.com/HOPLAB-LBP) | ☐ |
    | How to get started: behavioral study | Find guidance [here](../behaviour/index.md) | ☐ |
    | How to get started: EEG study | Find guidance [here](../eeg/index.md) | ☐ |
    | How to get started: fMRI study | Find guidance [here](../fmri/index.md) | ☐ |

??? note "🔬 Science outreach"
    | Task | Information | Done |
    |------|-------------|------|
    | Learn [how to register academic outreach](../../outreach.md#registering-your-academic-outreach-in-lirias) | Add the Methusalem label ([how?](https://research.kuleuven.be/en/associatienet/output/lirias/faq_webpages/labels-in-lirias)) | ☐ |
    | Learn how to register [non-academic outreach](../../outreach.md#straightforward-ideas-for-scicomm-participation) | Add them all [here](https://kuleuven.sharepoint.com/:x:/r/sites/T0005824/Shared%20Documents/General/Methusalem%20reporting/Methusalem_Scicomm_Overview.xlsx?d=wcafaa0336ef941b3989dd98a38b35e9b&csf=1&web=1&e=U3PAVl) | ☐ |
    | Check out our [science communication channels](../../outreach.md#social-media) | NeuroSPACE [LinkedIn](https://www.linkedin.com/company/neurospace-kuleuven/) and [Bluesky](https://bsky.app/profile/neurospacekuleuven.bsky.social) | ☐ |

??? note "🏛️ Practical lab culture"
    | Task | Information | Done |
    |------|-------------|------|
    | Get to know [relevant meeting structures](../../outreach.md#lab-meetings) | On the levels of Hoplab, B&C and NeuroSPACE | ☐ |
    | Get information on retreat and internal events | PPP, social lab activities, scicomm events | ☐ |
    | Be added to WhatsApp group for social stuff | Ask any colleague to add you! 😊 | ☐ |



